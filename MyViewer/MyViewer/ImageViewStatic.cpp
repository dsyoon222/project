		...
		if (!m_image.IsNull()) {
			...
			else 
			{
				...
				if (bOK) {
					...
					//DrawXMark(pDC, m_ptEnd, RGB(0, 0, 255),); //종료좌표 감춤
					image.ReleaseDC();
...
void CImageViewerWnd::DrawCircle(CDC *pDC)
{
	pDC->SelectStockObject(NULL_BRUSH);
	//int nOldMode = pDC->SetROP2(R2_NOT); //반전
	CRect rc(m_ptStart, m_ptStart);
	rc.InflateRect(m_nCircleRadius, m_nCircleRadius);
